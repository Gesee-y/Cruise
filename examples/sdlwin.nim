...
proc example5_modificateurs() =
  ...
  while running:
    ...
    # Shift+F5 → reload
    if (win.isKeyPressed(CKey_LShift) or win.isKeyPressed(CKey_RShift)) and win.isKeyJustPressed(CKey_F5):
...
proc example7_resize_reposition() =
  ...
  var win: SDL3Window
  new(win)
  app.initWindow(win, "Example 7 — Resize/Reposition", width=640, height=480,
                 posX=400, posY=200)
...
proc example8_multi_fenetres() =
  ...
  app.initWindow(w1, "Main Window",  width=800, height=600, posX=100, posY=100)
  app.initWindow(w2, "Secondary Window",  width=400, height=300, posX=950, posY=100)
  ...
  while running:
    ...
    if w1.isKeyJustPressed(CKey_F2):
      if w2Alive:
        ...
        new(w2)
        app.initWindow(w2, "Secondary Window (recreated)", width=400, height=300,
                       posX=950, posY=100)
        w2Alive = true
...
proc example10_mini_editeur() =
  ...
  var win: SDL3Window
  new(win)
  app.initWindow(win, "Mini Editor — F1 for help", width=900, height=600)
