...
when isMainModule:
  ...
  randomize()
  var app = initSDL3App()
  var window: SDL3Window
  
  new(window)
  app.initWindow(window, "SDL3 CRenderer Demo", posX=0, posY=0, width=W, height=H)
  
  ren = initSDLRenderer(window.handle)
  ...
  while running:
    ...
    if app.isKeyJustPressed(CKey_B):  enableBloom    = not enableBloom     ## B
    if app.isKeyJustPressed(CKey_V): enableVignette = not enableVignette  ## V
    if app.isKeyJustPressed(CKey_F):  enableFXAA     = not enableFXAA      ## F
    if app.isKeyJustPressed(CKey_C):  enableChroma   = not enableChroma    ## C
