...
import macros, types, fragment, archetypes, dense, sparse
...
template deleteEntityDefer*(world: var ECSWorld, d: DenseHandle,
  ...
  world.commandBufs[buffer_id].addCommand(DeleteOp.int, d.obj.archetypeId,
      0'u32, PayLoad(eid: d.obj.widx.uint, obj: d))
...
proc migrateEntity*(world: var ECSWorld, d: DenseHandle,
...
template migrateEntity*(world: var ECSWorld, ents: var openArray,
    archNode: ArchetypeNode) =
  if ents.len != 0:
