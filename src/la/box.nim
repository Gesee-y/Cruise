...
#############################################################################################################################
##
## Axis-Aligned Bounding CBoxes for 2D and 3D game development.
##
## Two concepts are used as the structural interface:
##
##   CBox2D — any type with x1,y1,x2,y2 : MFloat
##            Compatible with SDL_FRect and similar structs that store
##            two corner points directly.
##
##   CBox3D — any type with origin : Vec3  (min corner)
##                         size   : Vec3  (extent, always positive for valid Cbox)
##
## Concrete types CBoxF2 and CBoxF3 are provided ready to use.
...
func minX*[B: CBox2D](b: B): MFloat {.inline.} = b.x1
  ## Left edge (inclusive min X).
func minY*[B: CBox2D](b: B): MFloat {.inline.} = b.y1
  ## Bottom edge (inclusive min Y).
func maxX*[B: CBox2D](b: B): MFloat {.inline.} = b.x2
  ## Right edge (inclusive max X).
func maxY*[B: CBox2D](b: B): MFloat {.inline.} = b.y2
  ## Top edge (inclusive max Y).

func width*[B: CBox2D](b: B): MFloat {.inline.} =
  ## Width of the Cbox (x2 - x1).
  b.x2 - b.x1

func height*[B: CBox2D](b: B): MFloat {.inline.} =
  ## Height of the Cbox (y2 - y1).
  b.y2 - b.y1

func centerX*[B: CBox2D](b: B): MFloat {.inline.} =
  ## X coordinate of the center.
  (b.x1 + b.x2) * 0.5f

func centerY*[B: CBox2D](b: B): MFloat {.inline.} =
  ## Y coordinate of the center.
  (b.y1 + b.y2) * 0.5f

func area*[B: CBox2D](b: B): MFloat {.inline.} =
  ## Surface area of the 2D Cbox.
  b.width * b.height

func perimeter*[B: CBox2D](b: B): MFloat {.inline.} =
  ## Perimeter of the 2D Cbox.
  2f * (b.width + b.height)

func isEmpty*[B: CBox2D](b: B): bool {.inline.} =
  ## True when the Cbox has zero or negative area.
  b.x2 <= b.x1 or b.y2 <= b.y1

# ─────────────────────────────────────────────────────────────────── CBox3D ───

func minCorner*[B: CBox3D](b: B): auto {.inline.} =
  ## Min corner (= origin).
  b.origin

func maxCorner*[B: CBox3D](b: B): auto {.inline.} =
...
func center*[B: CBox3D](b: B): auto {.inline.} =
  ## Center point of the 3D Cbox.
  type V = typeof(b.origin)
  V(x: b.origin.x + b.size.x*0.5f,
    y: b.origin.y + b.size.y*0.5f,
    z: b.origin.z + b.size.z*0.5f)

func volume*[B: CBox3D](b: B): MFloat {.inline.} =
  ## Volume of the 3D Cbox (w * h * d).
  b.size.x * b.size.y * b.size.z

func surfaceArea*[B: CBox3D](b: B): MFloat {.inline.} =
  ## Surface area = 2(wh + wd + hd).
  2f * (b.size.x*b.size.y + b.size.x*b.size.z + b.size.y*b.size.z)

func isEmpty*[B: CBox3D](b: B): bool {.inline.} =
...
func `==`*[B: CBox2D](a, b: B): bool {.inline.} =
  a.x1 == b.x1 and a.y1 == b.y1 and a.x2 == b.x2 and a.y2 == b.y2

func `==`*[B: CBox3D](a, b: B): bool {.inline.} =
...
func approxEq*[B: CBox2D](a, b: B, eps: MFloat = 1e-6f): bool {.inline.} =
  abs(a.x1-b.x1)<=eps and abs(a.y1-b.y1)<=eps and
  abs(a.x2-b.x2)<=eps and abs(a.y2-b.y2)<=eps

func approxEq*[B: CBox3D](a, b: B, eps: MFloat = 1e-6f): bool {.inline.} =
...
func contains*[B: CBox2D](b: B, px, py: MFloat): bool {.inline.} =
  ## True when point (px,py) is strictly inside the Cbox (not touching edges).
  px > b.x1 and px < b.x2 and py > b.y1 and py < b.y2

func isTouching*[B: CBox2D](b: B, px, py: MFloat): bool {.inline.} =
  ## True when point (px,py) is inside or on the boundary.
  px >= b.x1 and px <= b.x2 and py >= b.y1 and py <= b.y2

func containsCBox*[B: CBox2D](outer, inner: B): bool {.inline.} =
...
func overlaps*[B: CBox2D](a, b: B): bool {.inline.} =
  ## True when the two Cboxes share any area (touching edges count).
  a.x1 <= b.x2 and a.x2 >= b.x1 and a.y1 <= b.y2 and a.y2 >= b.y1

func overlapsExclusive*[B: CBox2D](a, b: B): bool {.inline.} =
  ## True when the two Cboxes share area, but NOT if they merely touch at edges.
  a.x1 < b.x2 and a.x2 > b.x1 and a.y1 < b.y2 and a.y2 > b.y1

# ─────────────────────────────────────────────────────────────────── CBox3D ───

func contains*[B: CBox3D](b: B, px, py, pz: MFloat): bool {.inline.} =
  ## True when point (px,py,pz) is strictly inside the Cbox.
...
func isTouching*[B: CBox3D](b: B, px, py, pz: MFloat): bool {.inline.} =
  ## True when point (px,py,pz) is inside or on the boundary.
  px >= b.origin.x and px <= b.origin.x+b.size.x and
  py >= b.origin.y and py <= b.origin.y+b.size.y and
  pz >= b.origin.z and pz <= b.origin.z+b.size.z

func containsPoint*[B: CBox3D, V](b: B, p: V): bool {.inline.} =
  ## True when Vec3-like point p is strictly inside the Cbox.
  isTouching(b, p.x, p.y, p.z)

func containsBox*[B: CBox3D](outer, inner: B): bool {.inline.} =
...
func overlaps*[B: CBox3D](a, b: B): bool {.inline.} =
  ## True when the two 3D Cboxes share any volume (touching faces count).
...
func overlapsExclusive*[B: CBox3D](a, b: B): bool {.inline.} =
  ## True when the two 3D Cboxes share volume, NOT when merely touching.
  a.origin.x < b.origin.x+b.size.x and a.origin.x+a.size.x > b.origin.x and
  a.origin.y < b.origin.y+b.size.y and a.origin.y+a.size.y > b.origin.y and
  a.origin.z < b.origin.z+b.size.z and a.origin.z+a.size.z > b.origin.z
...
func closestPoint*[B: CBox2D](b: B, px, py: MFloat): (MFloat, MFloat) {.inline.} =
  ## Closest point on or inside the Cbox to (px,py).
  (clamp(px, b.x1, b.x2), clamp(py, b.y1, b.y2))

func distanceSq*[B: CBox2D](b: B, px, py: MFloat): MFloat {.inline.} =
  ## Squared distance from point (px,py) to the Cbox surface (0 if inside).
  let dx = max(0f, max(b.x1-px, px-b.x2))
  let dy = max(0f, max(b.y1-py, py-b.y2))
  dx*dx + dy*dy

func distance*[B: CBox2D](b: B, px, py: MFloat): MFloat {.inline.} =
  ## Distance from point (px,py) to the Cbox surface (0 if inside).
  sqrt(distanceSq(b, px, py))

func closestPoint*[B: CBox3D, V](b: B, p: V): V {.inline.} =
  ## Closest point on or inside the 3D Cbox to Vec3-like point p.
...
func distanceSq*[B: CBox3D, V](b: B, p: V): MFloat {.inline.} =
  ## Squared distance from Vec3-like point p to the 3D Cbox surface.
  let mx = b.origin.x+b.size.x
  let my = b.origin.y+b.size.y
  let mz = b.origin.z+b.size.z
...
func distance*[B: CBox3D, V](b: B, p: V): MFloat {.inline.} =
  ## Distance from Vec3-like point p to the 3D Cbox surface (0 if inside).
  sqrt(distanceSq(b, p))
...
func intersect*[B: CBox2D](a, b: B): B {.inline.} =
  ## Intersection of two CBox2D values.
...
func merge*[B: CBox2D](a, b: B): B {.inline.} =
  ## Smallest Cbox containing both a and b (union / bounding Cbox).
  B(x1: min(a.x1, b.x1), y1: min(a.y1, b.y1),
    x2: max(a.x2, b.x2), y2: max(a.y2, b.y2))

func expandBy*[B: CBox2D](b: B, amount: MFloat): B {.inline.} =
  ## Grow the Cbox by `amount` on all sides.
  B(x1: b.x1-amount, y1: b.y1-amount,
    x2: b.x2+amount, y2: b.y2+amount)

func shrinkBy*[B: CBox2D](b: B, amount: MFloat): B {.inline.} =
  ## Shrink the Cbox by `amount` on all sides (may produce empty Cbox).
  B(x1: b.x1+amount, y1: b.y1+amount,
    x2: b.x2-amount, y2: b.y2-amount)

func translate*[B: CBox2D](b: B, dx, dy: MFloat): B {.inline.} =
  ## Translate the Cbox by (dx, dy).
  B(x1: b.x1+dx, y1: b.y1+dy, x2: b.x2+dx, y2: b.y2+dy)

func scale*[B: CBox2D](b: B, sx, sy: MFloat): B {.inline.} =
  ## Scale the Cbox around the origin by (sx, sy).
  B(x1: b.x1*sx, y1: b.y1*sy, x2: b.x2*sx, y2: b.y2*sy)

func enclosingPoint*[B: CBox2D](b: B, px, py: MFloat): B {.inline.} =
...
func intersect*[B: CBox3D](a, b: B): B =
  ## Intersection of two CBox3D values.
...
func merge*[B: CBox3D](a, b: B): B =
  ## Smallest Cbox containing both a and b.
  let
    ax2 = a.origin.x+a.size.x
    ay2 = a.origin.y+a.size.y
    az2 = a.origin.z+a.size.z
...
func expandBy*[B: CBox3D](b: B, amount: MFloat): B {.inline.} =
  ## Grow the 3D Cbox by `amount` on all sides.
  type V = typeof(b.origin)
...
func shrinkBy*[B: CBox3D](b: B, amount: MFloat): B {.inline.} =
  ## Shrink the 3D Cbox by `amount` on all sides (may produce empty Cbox).
  type V = typeof(b.origin)
  B(origin: V(x:b.origin.x+amount, y:b.origin.y+amount, z:b.origin.z+amount),
...
func translate*[B: CBox3D, V](b: B, delta: V): B {.inline.} =
  ## Translate the 3D Cbox by Vec3-like delta.
  type Ov = typeof(b.origin)
  B(origin: Ov(x:b.origin.x+delta.x, y:b.origin.y+delta.y, z:b.origin.z+delta.z),
    size:   b.size)

func enclosingPoint*[B: CBox3D, V](b: B, p: V): B {.inline.} =
...
## Returns (tMin, tMax, hit) where hit=true means the ray intersects the Cbox.
...
func rayIntersectVec*[B: CBox3D, V](b: B, origin, dir: V, tMin=0f, tMax=1e30f): RayHit3D {.inline.} =
  ## Vec3-friendly overload of rayIntersect for CBox3D.
...
func corners*[B: CBox2D](b: B): array[4, (MFloat,MFloat)] {.inline.} =
  ## The 4 corners of a 2D Cbox in counter-clockwise order starting from min.
  [(b.x1,b.y1), (b.x2,b.y1), (b.x2,b.y2), (b.x1,b.y2)]

func corners*[B: CBox3D](b: B): array[8, (MFloat,MFloat,MFloat)] =
  ## All 8 corners of a 3D Cbox.
...
func transformedAabb*[B: CBox3D](b: B, m00,m01,m02,m10,m11,m12,m20,m21,m22: MFloat): B =
  ## Compute the AABB of a CBox3D after applying a 3×3 linear transform (rotation+scale).
...
func `$`*[B: CBox2D](b: B): string =
  "CBox2D(min=(" & $b.x1 & "," & $b.y1 & ") max=(" & $b.x2 & "," & $b.y2 &
  ") size=(" & $b.width & "," & $b.height & "))"

func `$`*[B: CBox3D](b: B): string =
  "CBox3D(origin=(" & $b.origin.x & "," & $b.origin.y & "," & $b.origin.z &
