...
suite "Sparse ECS logic":
  ...
  test "allocate single sparse entity":
    ...
    let e = w.createTSparseEntity(Pos)
    check e.id == 0
  ...
  test "allocate multiple sparse entities sequential":
    ...
    let ents = w.createTSparseEntities(10, Pos)
    check ents.len == 10
  ...
  test "delete sparse entity and reuse id":
    ...
    var e1 = w.createTSparseEntity(Pos)
    var e2 = w.createTSparseEntity(Pos)

    w.deleteEntity(e1)

    let e3 = w.createTSparseEntity(Pos)
    check e3.id == e1.id
  ...
  test "activate and deactivate sparse component":
    ...
    var e = w.createTSparseEntity(Pos)
    var re = w.addComponent(e, Vel)
    ...
    check pmask[][0] == true
    check vmask[][0] == true
    discard w.removeComponent(re, Vel)
  ...
  test "batch allocate activates all bits":
    ...
    let ents = w.createTSparseEntities(64, Pos)
    let mask = w.registry.entries[pid].getSparseMaskOp(
      w.registry.entries[pid].rawPointer)
  ...
  test "batch activation works":
    ...
    let ents = w.createTSparseEntities(32, Pos)
    var ids: seq[uint]
  ...
  test "free_list integrity after deletes":
    ...
    var ents = w.createTSparseEntities(10, Pos)
    for i in 0..<ents.len:
      var e = ents[i]
      w.deleteEntity(e)
