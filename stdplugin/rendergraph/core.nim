...
include "../../src/plugins/plugins.nim"          # Plugin, PluginNode, attachSystem, addDependency …
include "../../src/render/render.nim"
...
proc reachesBackbuffer(
  ...
  for edge in plugin.graph.outedges[nodeId]:
...
proc compile*(rg: var RenderGraph) =
  ...
  let levels = rg.plugin.parallel_cache   # seq[array[2, seq[int]]]
...
proc executeFrame*[R](rg: var RenderGraph, renderer: var R) =
  ...
  let levels = rg.plugin.parallel_cache
