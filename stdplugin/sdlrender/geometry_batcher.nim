...
proc emitRect*(gb:        var GeometryBatcher,
  ...
  if filled:
    ...
    gb.emitLine(fpoint(x0,y0), fpoint(x1,y0), color, targetKey, blend, thickness)  # top
    gb.emitLine(fpoint(x1,y0), fpoint(x1,y1), color, targetKey, blend, thickness)  # right
    gb.emitLine(fpoint(x1,y1), fpoint(x0,y1), color, targetKey, blend, thickness)  # bottom
    gb.emitLine(fpoint(x0,y1), fpoint(x0,y0), color, targetKey, blend, thickness)  # left
...
proc emitCircle*(gb:        var GeometryBatcher,
  ...
  if filled:
    ...
    var iPrev = b.addVertex(Vertex(pos: fpoint(center.x + radius, center.y), color: color))
    ...
    var iOutPrev  = b.addVertex(Vertex(pos: fpoint(center.x + fringeR, center.y), color: alphaZero))
...
proc emitTexturedQuad*(gb:         var GeometryBatcher,
  ...
  for k in 0..3:
    ...
    indices[k] = b.addVertex(Vertex(pos: fpoint(rx, ry), uv: uvs[k], color: tint))
